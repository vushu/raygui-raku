...
grammar Grammar {
    ...
    rule macro-block {
        | <extern>
        | <not-defined-macro>
        | <if-raygui-standalone>
        | <if-raygui-implementation>
        | <if-macro-block>
        | <ifndef-block>
        | <else-macro-line>
        | <elif-macro-line>
    ...
    rule not-defined-macro {
        '#if' '!defined' '(' <identifier> ')' <defined-content>* '#endif'
    }
    ...
    rule if-raygui-implementation {
        '#if' 'defined(RAYGUI_IMPLEMENTATION)' .*? '#endif' '// RAYGUI_IMPLEMENTATION'
    }

    rule if-raygui-standalone {
        # Quite hackish to ignore standalone, this needs be changed at some point.
        '#if' 'defined(RAYGUI_STANDALONE)' .* '#endif' '// Style property'
    }
    ...
    rule typedef-enum-bool {
        'typedef' 'enum' <block> bool ';'
    }
    ...
    rule define-decl {
        ...
        # | '#define' <api-decl> '__declspec' '(' <identifier> ')' <comment>?
    }
